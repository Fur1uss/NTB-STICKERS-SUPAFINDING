...
import GameServiceDirect from '../../services/gameServiceDirect';
import soundService from '../../services/soundService';
import uploadServiceDirect from '../../services/uploadServiceDirect';
...
const ScoreboardScreen = () => {
  ...
  const loadScoreboardData = async () => {
    ...
    try {
      ...
      const scoreboardData = await GameServiceDirect.getScoreboardData(gameId);
      setGameData(scoreboardData);

      // Cargar ranking global
      const backendUser = JSON.parse(localStorage.getItem('backendUser') || '{}');
      const ranking = await GameServiceDirect.getGlobalRanking({
        limit: 10,
        page: 1,
  ...
  const proceedWithUpload = async () => {
    ...
    try {
      ...
      const fileName = file.name || 'sticker.png';
      const result = await uploadServiceDirect.uploadSticker(file, userId, name, description, fileName);
      
      if (!result.success) {
        throw new Error(result.message || 'Error al subir el sticker');
      }
